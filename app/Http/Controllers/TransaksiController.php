...
class TransaksiController extends Controller
{
    ...
    public function create(Request $request, Member $member)
    {
        ...
        $pakets = Paket::where('outlet_id', $outlet->id)
            ->select('id as value', 'nama_paket as option')
            ->get();
    ...
    public function store(Request $request, Member $member)
    {
        ...
        if (Cart::session($member->id)->isEmpty()) {
            return back()->withErrors(['paket' => 'Paket tidak boleh kosong']);
        }
        ...
        if($uang_tunai && ($kembalian < 0)) {
            return back()->withInput()->withErrors(['uang_tunai' => 'Uang tunai kurang dari total bayar']);
        }
    ...
    {
        LogActivity::add('Berhasil Mengubah Transaksi');
        $request->validate([
            'diskon' => 'nullable|numeric|min:0|',
            'biaya_tambahan' => 'nullable|numeric',
            'uang_tunai' => 'nullable|numeric',
